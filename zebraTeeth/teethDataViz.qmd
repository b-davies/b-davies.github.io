...
ggplot(Ex1_Amp3,aes(x=mm_band,y=d18O,color=model)) + geom_line() +
   geom_hline(yintercept=-4,linetype=2)+
  geom_line(lwd=1.1) +
...
```{r}
Ex4_high<-Ex4 %>%  filter(model=="High")
...
```{r}
cutoff<-20
Ex4_highCut<-Ex4 %>%  filter(model=="High") %>% group_by(specimen) %>% filter( n() >=cutoff) %>% ungroup()
